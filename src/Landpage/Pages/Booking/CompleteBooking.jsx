...
import { FaCheckCircle, FaTimesCircle, FaPercent, FaShareAlt, FaHome, FaListAlt } from "react-icons/fa";
...
const CompleteBooking = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // حالة التحميل للزرار
  const [isSuccess, setIsSuccess] = useState(false); // حالة النجاح لظهور رسالة التأكيد
  const [generatedPdfFile, setGeneratedPdfFile] = useState(null); // لحفظ الـ PDF للمشاركة
  ...
  const bookingData = state || JSON.parse(localStorage.getItem("bookingData")) || {};
  const {
    adults = 0,
    children = 0,
    infants = 0,
  ...
  const generateBookingPDF = (apiData, localBookingData, tourDetails) => {
    ...
    doc.text(`Tour Name: ${tourDetails.title || "N/A"}`, 14, 38);
    doc.text(`Location: ${tourDetails.country || "N/A"}, ${tourDetails.city || "N/A"}`, 14, 46);
    
    const policyText = `Policy: ${tourDetails.policy || "N/A"}`;
    const splitPolicy = doc.splitTextToSize(policyText, 180); 
    doc.text(splitPolicy, 14, 54); 

    let currentY = 54 + (splitPolicy.length * 7); 
    doc.text(`Contact: 01035555313`, 14, currentY);
    currentY += 10; 
    ...
    const pricingData = [
      ["Adults", `${localBookingData?.adults || 0} x $${tourDetails.price?.adult || 0}`, `$${localBookingData?.adultsTotal || 0}`],
      ["Children", `${localBookingData?.children || 0} x $${tourDetails.price?.child || 0}`, `$${localBookingData?.childrenTotal || 0}`],
      ["Infants", `${localBookingData?.infants || 0} x $${tourDetails.price?.infant || 0}`, `$${localBookingData?.infantsTotal || 0}`],
    ];
    ...
    const fileName = `Booking_${tourDetails.title}_${new Date().getTime()}.pdf`;
    
    // 1. تنزيل الملف بشكل مباشر
    doc.save(fileName);

    // 2. استخراج الملف كمحاولة لمشاركته لاحقاً
    const pdfBlob = doc.output('blob');
    const fileForSharing = new File([pdfBlob], fileName, { type: 'application/pdf' });
    return fileForSharing;
  };
    ...
    const fetchData = async () => {
      try {
        ...
        if (tourData?.meetingPointAddress && tourData?.meetingPointLocation) {
          ...
          if (typeof tourData.meetingPointLocation === "string" && tourData.meetingPointLocation.includes("maps?q=")) {
            const coords = tourData.meetingPointLocation.split("maps?q=")[1].split(",");
            SetMeetingPointLocation({ lat: parseFloat(coords[0]), lng: parseFloat(coords[1]) });
          } else {
  ...
  useEffect(() => {
    const fetchPaymentMethods = async () => {
      try {
        const res = await axios.get("https://bcknd.tickethub-tours.com/api/user/landPage/active");
  ...
  const calculateFinalTotal = () => {
    if (!discountcode || !discountType) return total;
    let finalAmount;
  ...
  const handleApply = async () => {
    ...
    if (!code) {
      toast.warn("You should write a code");
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) return toast.warn("You need to login first.");
    if (!selectedPayment) return toast.warn("Please select a payment method.");
    if (!description) return toast.warn("Address/Description is required.");
    if (!meetingPointLocation) return toast.warn("Meeting Point Location is required.");
    if (!image) return toast.warn("Payment receipt image is required.");

    setIsSubmitting(true); // تشغيل حالة التحميل
    ...
    try {
      const payload = {
        ...
        proofImage: image,
        discount: Number(adultsDiscount), // يمكنك تعديلها لاحقاً لو احتجت تجمع باقي الخصومات
        address: description,
        location: `https://www.google.com/maps?q=${meetingPointLocation.lat},${meetingPointLocation.lng}`,
      };

      if (codeid) payload.promoCodeId = codeid;
      ...
      if (apiData?.success) {
        const pdfFile = generateBookingPDF(res.data.data, bookingData, data);
        setGeneratedPdfFile(pdfFile);
        localStorage.removeItem("bookingData");
        setIsSuccess(true); // تفعيل صفحة النجاح
      }

    } catch (error) {
      const err = error?.response?.data?.error || error?.response?.error || error?.response?.data || error;
      if (err?.details && Array.isArray(err.details)) {
        err.details.forEach((detail) => toast.error(`${detail.field}: ${detail.message}`));
      } else if (Array.isArray(err?.errors)) {
        err.errors.forEach((e) => toast.error(e.message || e));
  ...
  const handleSharePdf = async () => {
    if (navigator.share && generatedPdfFile) {
      ...
      toast.info("Your device or browser doesn't support direct file sharing.");
    }
  };

  if (loading) return <div className="h-screen w-screen"><Loading /></div>;
  if (!data) { navigate(-1); return null; }

  // ------------------------------------------------------------------
  // واجهة النـجــــاح (Success UI)
  // ------------------------------------------------------------------
  if (isSuccess) {
    return (
      <div>
...
Your booking has been successfully confirmed. The PDF file containing your booking details has been automatically downloaded to your device.
          </p>

          <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
            <button 
              onClick={handleSharePdf} 
              className="flex items-center justify-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:bg-blue-700 transition active:scale-95"
            >
              <FaShareAlt /> Share PDF
            </button>
            <button 
              onClick={() => navigate('/profileuser')} // عدل المسار ده لمسار حجوزاتك الحقيقي
              className="flex items-center justify-center gap-2 bg-one text-white px-6 py-3 rounded-xl font-semibold shadow-lg hover:bg-one/90 transition active:scale-95"
            >
              <FaListAlt /> View My Bookings
            </button>
            <button 
              onClick={() => navigate('/')} 
              className="flex items-center justify-center gap-2 bg-gray-200 text-gray-800 px-6 py-3 rounded-xl font-semibold shadow hover:bg-gray-300 transition active:scale-95"
            >
              <FaHome /> Back to Home
            </button>
          </div>
    ...
    <div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 px-4 py-8 max-w-7xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="w-full lg:w-1/2 bg-white text-white p-6 rounded-xl border border-gray-700">
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                type="button"
                disabled={loading}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-one text-white hover:bg-one/90 disabled:bg-gray-400"
              >
                Apply <MdLocalOffer className="text-lg" />
              </button>
            </div>
            ...
            <div className="flex flex-col gap-2 mt-3 text-sm">
              {errorcode && (
                <span className="flex items-center gap-2 text-red-600">
                  <FaTimesCircle className="text-lg" /> {errorcode}
                </span>
              )}
              {discountcode && (
                <span className="flex items-center gap-2 text-green-600">
                  <FaCheckCircle className="text-lg" /> You will get a discount: {discountcode}
                </span>
              )}
              {discountType && (
                <span className="flex items-center gap-2 text-green-600">
                  <FaPercent className="text-lg" /> Discount Type: {discountType}
                </span>
              )}
            </div>
          </div>
          ...
          {hasMeetingPointFromApi ? (
            <>
              ...
              <input type="text" value={description} readOnly className="w-full rounded-xl border border-gray-300 bg-gray-100 px-4 py-2 text-gray-800" />
              <MapPicker location={meetingPointLocation} readOnly />
            </>
          ) : (
            <>
              <label className="block text-sm font-medium text-gray-700 mt-4 mb-1">Address</label>
              <input type="text" placeholder="Description of the Address" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full rounded-xl border border-gray-300 bg-white px-4 py-2 text-gray-800" />
              <div className="my-3">
            ...
            {paymentOptions.map((method) => (
              <button
                ...
                className={`flex flex-col items-center justify-center rounded-lg p-2 border transition transform ${
                  selectedPayment === method.id ? "border-one scale-105 bg-blue-50" : "border-gray-300"
                }`}
              >
          ...
          <FileUploadButtontype onFileChange={setimage} pic={image} des={`Payment receipt image`} />

          <button
            type="submit"
            disabled={isSubmitting} // تعطيل الزرار لو بنعمل submit
            className={`mt-4 w-full flex items-center justify-center gap-2 py-3 px-4 rounded-2xl font-semibold text-white shadow-lg transition-all duration-200 ease-out 
              ${isSubmitting ? "bg-one/70 cursor-not-allowed" : "bg-one hover:bg-one/95 hover:-translate-y-0.5 hover:shadow-xl active:scale-[0.98]"}`}
          >
            {isSubmitting ? (
              <>
        ...
        <div className="w-full lg:w-1/2 bg-gray-200 h-fit py-10 rounded-3xl p-6 shadow-lg">
          ...
          <div className="text-sm text-gray-500 mb-4">
            Currency: <span className="text-one font-medium">{data.price.currency}</span>
          </div>
          ...
          <div className="mt-6 border-t pt-4">
            ...
            {adultsDiscount > 0 && (
              <div className="flex justify-between text-sm py-1 text-red-500">
                <span>Adult Discount:</span><span>-${adultsDiscount.toFixed(2)}</span>
              </div>
            )}
            ...
            {childrenDiscount > 0 && (
              <div className="flex justify-between text-sm py-1 text-red-500">
                <span>Children Discount:</span><span>-${childrenDiscount.toFixed(2)}</span>
              </div>
            )}
            ...
            {infantsDiscount > 0 && (
              <div className="flex justify-between text-sm py-1 text-red-500">
                <span>Infants Discount:</span><span>-${infantsDiscount.toFixed(2)}</span>
              </div>
            )}
            ...
            <div className="flex flex-col gap-2 mt-3 border-t pt-4">
              ...
              {discountcode && (
                <div className="flex justify-between text-base text-green-600">
                  ...
                  <span>- {discountType === "amount" ? `$${discountcode}` : `${discountcode}%`}</span>
                </div>
              )}
              <div className="flex justify-between text-lg font-bold text-one">
                <span>Final Total:</span><span>${calculateFinalTotal().toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
