...
const FileUploadButtontype = ({ onFileChange, kind,des, pic ,like}) => {
  ...
  {!like&&   <span className='font-bold text-one'>{kind}</span>}  
      ...
      <button
      type='button'
      className='w-full h-[150px] border border-one rounded-[8px] flex justify-center items-center px-5'
        onClick={() => document.getElementById(`file-upload-${kind}`).click()}
      >
        <div className='flex justify-center items-center w-full h-[100px]  gap-2 mt-2'>
          {showed ? (
            <img
              className='w-25 h-[100px] object-cover rounded'
              src={
            ...
            <span className='text-one text-sm '>No Image</span>
                    {!showed && <MdOutlineCloudDownload className='w-20 h-20 ' />}
          </>
...
export default FileUploadButtontype;
