...
import { LuClock, LuMapPin, LuCheckCheck, LuFileText, LuGem } from "react-icons/lu"; // أضفت CiViewColumn للخطأ
...
const InfoTour = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // التحقق من استجابة السيرفر بناءً على الهيكل الذي أرسلته
        if (res.data.success) {
          setData(res.data.data);
        } else {
          // في حال أرسل السيرفر success: false كـ Response ناجح (مثل 200 OK مع رسالة خطأ)
          setError(true);
        }
      } catch (err) {
        // في حال فشل الطلب تماماً (مثل 404 الفعلي من السيرفر)
  ...
  // واجهة الخطأ باللغة الإنجليزية
  if (error) {
    return (
      <div className="bg-[#F8FAFC] min-h-screen flex flex-col">
        <Navtwo />
        <div className="flex-1 flex flex-col items-center justify-center px-4 text-center">
          <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl max-w-md border border-slate-100 animate-in zoom-in duration-300">
            <div className="bg-red-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6">
              <CiViewColumn className="text-red-500 text-5xl" />
            </div>
            <h2 className="text-3xl font-black text-slate-800 mb-4">Tour Not Found</h2>
            <p className="text-slate-500 mb-8 text-lg leading-relaxed">
              We're sorry, but the tour you are looking for is no longer available or could not be found. 
              Please check the URL or return to our tours list.
            </p>
            <button
              onClick={() => navigate('/')}
              className="w-full bg-one text-white py-4 rounded-2xl font-bold text-lg hover:shadow-xl hover:bg-one/90 transition-all"
            >
              Back to Tours
            </button>
          </div>
  ...
  if (loading) return <div className="h-screen w-screen flex items-center justify-center bg-white"><Loading /></div>;
  if (!data) return null; // حماية إضافية
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen font-sans text-slate-900">
      ...
      <div className=" mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h1 className="text-3xl md:text-5xl font-black text-slate-900 mb-8 leading-tight italic">
          {data.title}
        </h1>
          ...
          <div className="sticky top-28 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm">
               ...
               <div className="p-4 bg-slate-50 rounded-2xl text-sm text-slate-500 leading-relaxed italic border-l-4 border-one">
                 "{data.policy || "Standard cancellation policies apply to this tour."}"
               </div>
            </div>
